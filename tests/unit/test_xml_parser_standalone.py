"""
Standalone unit tests for XML parser bypassing plugin system.
"""
import sys
import os
import unittest
...
# Create the XMLParser class directly for testing
class XMLParser:
    ...
    def parse_schema(self, filename: str, content: bytes) -> SchemaDetails:
        ...
        try:
            ...
            print(f"Error parsing XML schema: {str(e)}")
    ...
    def _infer_schema_from_xml(self, root) -> Tuple[List[FieldInfo], List[str]]:
        ...
        while elements_to_process:
            element, parent_path = elements_to_process.pop(0)  # Use pop(0) for breadth-first traversal
...
# XML Parser instance for testing
xml_parser = XMLParser()


class TestXMLParser(unittest.TestCase):
    """Test case for the standalone XML parser."""
    ...
    def test_parse_schema_xml(self):
        ...
        # Validate fields exist - check common element paths
        fields = {f.path: f for f in schema.fields}
        self.assertTrue(any('users.user' in path for path in fields.keys()), 
                       "Expected 'users.user' prefix in fields")
